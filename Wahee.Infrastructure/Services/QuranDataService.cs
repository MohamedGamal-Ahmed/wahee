...
namespace Wahee.Infrastructure.Services
{
    public class QuranDataService : IQuranDataService
    {
        ...
        // FIXED: Force HTTPS for external API endpoints
        private const string RadiosApiBaseUrl = "https://mp3quran.net/api/v3/";
        private const string RadiosApiPath = "radios?language=ar";
        ...
        public async Task RefreshRadiosAsync()
        {
            try
            {
                var url = new Uri(new Uri(RadiosApiBaseUrl), RadiosApiPath);
                var response = await _httpClient.GetFromJsonAsync<RadiosResponse>(url);
